...
if (!isset($_GET['id']) || empty($_GET['id'])) {
    $_SESSION['error_msg'] = "No class selected for editing.";
...
$class_query = $conn->prepare("SELECT * FROM classes WHERE id = ?");
$class_query->bind_param("i", $class_id);
$class_query->execute();
$class_result = $class_query->get_result();

if ($class_result->num_rows === 0) {
    $_SESSION['error_msg'] = "Class not found.";
    header("Location: manage_class.php");
    exit();
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['update_class'])) {
    ...
    $check_sql = "SELECT id FROM classes WHERE class_name = ? AND grade = ? AND id != ?";
    $check_stmt = $conn->prepare($check_sql);
    $check_stmt->bind_param("ssi", $class_name, $grade, $class_id);
    $check_stmt->execute();
    $check_result = $check_stmt->get_result();
    
    if ($check_result->num_rows > 0) {
        $_SESSION['error_msg'] = "Class name already exists for this grade.";
    } else {
        // Update class
        $update_sql = "UPDATE classes SET class_name = ?, grade = ? WHERE id = ?";
        $update_stmt = $conn->prepare($update_sql);
        $update_stmt->bind_param("ssi", $class_name, $grade, $class_id);
        
        if ($update_stmt->execute()) {
            $_SESSION['success_msg'] = "Class updated successfully!";
            header("Location: manage_class.php");
            exit();
        } else {
            $_SESSION['error_msg'] = "Error updating class: " . $conn->error;
        }
    }
}
?>
...
<head>
    ...
    <title>Edit Class - e-RPH Mobile</title>
    <!-- Bootstrap CSS -->
                    ...
                    <div class="row mb-3">
                        <div class="col-md-6">
                            ...
                            <input type="text" class="form-control" id="class_name" name="class_name" 
                                   value="<?php echo htmlspecialchars($class_info['class_name']); ?>" required>
                        </div>
                        <div class="col-md-6">
                            <label for="grade" class="form-label required-field">Form</label>
                            <select class="form-control" id="grade" name="grade" required>
                                <option value="">Select Form</option>
                                <option value="Lower Form" <?php echo $class_info['grade'] === 'Lower Form' ? 'selected' : ''; ?>>Lower Form</option>
                                <option value="Upper Form" <?php echo $class_info['grade'] === 'Upper Form' ? 'selected' : ''; ?>>Upper Form</option>
                            </select>
                        </div>
                    </div>
                    <div class="text-end">
                        <a href="manage_class.php" class="btn btn-secondary me-2">
